...
''';
  ...
  Future<StructuredNoteData?> structureNote(
    ...
    try {
      ...
      final referenceDate = noteDate ?? DateTime.now();
      ...
      final prompt =
          '''
Дата создания заметки (точка отсчета): ${referenceDate.toIso8601String()}.
Строго используй ЭТУ дату как "сегодня" для высчитывания всех относительных дат 
...
Для КАЖДОЙ заметки используй её поле "createdAt" как точку отсчета (сегодня) для высчитывания всех относительных дат в тексте этой заметки.
(например: если createdAt = 2024-05-10, а в тексте "завтра", то дата события должна быть 2024-05-11).
